...
// Inline micro-behaviors from shared package to avoid import issues
// These match packages/shared/src/micro-behaviors.ts exactly
const MICRO_BEHAVIORS = [
  ...
  },
  {
    ...
    description:
      "Thank someone specifically for raising a concern or disagreeing.",
    tier: 1,
    rank: 3,
  ...
  },
  {
    ...
    description:
      'Respond to bad news with "Help me understand" not "How did this happen?"',
    tier: 1,
    rank: 4,
  ...
  // TIER 2 — Deepen (Days 30-90)
  {
    ...
    description:
      'Ask "What are we missing?" before making a final decision.',
    tier: 2,
    rank: 6,
  ...
  },
  {
    ...
    description:
      "Share a personal mistake or learning moment with your team.",
    tier: 2,
    rank: 7,
    scarfDomains: ["status", "relatedness"],
    category: "vulnerability",
  ...
  },
  {
    ...
    description:
      "Check in on the person, not the project. Ask how they're doing, not what they're doing.",
    tier: 2,
    rank: 8,
  ...
  },
  {
    ...
    description:
      "Credit someone else's idea publicly in a meeting or message.",
    tier: 2,
    rank: 9,
    scarfDomains: ["status", "fairness"],
    category: "acknowledgment",
  ...
  },
  {
    ...
    description:
      "Explicitly separate brainstorming from evaluation — make it clear when ideas are being generated vs. judged.",
    tier: 2,
    rank: 10,
...
export default async function libraryRoutes(app: FastifyInstance) {
  // GET / — all micro-behaviors grouped by tier
  app.get("/", async (request: FastifyRequest, reply: FastifyReply) => {
    const grouped: Record<number, typeof MICRO_BEHAVIORS> = {};

    for (const mb of MICRO_BEHAVIORS) {
      if (!grouped[mb.tier]) {
        grouped[mb.tier] = [];
      }
      grouped[mb.tier].push(mb);
    }

    return { behaviors: grouped };
  });

  // GET /:id — single micro-behavior
  app.get<{ Params: { id: string } }>(
    "/:id",
    async (request, reply) => {
      const { id } = request.params;
      const behavior = MICRO_BEHAVIORS.find((mb) => mb.id === id);

      if (!behavior) {
        return reply.status(404).send({ error: "Micro-behavior not found" });
      }
